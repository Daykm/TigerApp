...
public class OAuth2Interceptor implements Interceptor {
	...
	static final String[] OAUTH_SIGNING_VALUES = {
			OAUTH_CALLBACK, OAUTH_CONSUMER_KEY, OAUTH_NONCE, OAUTH_SIGNATURE_METHOD, OAUTH_TIMESTAMP,
	...
	@Override public Response intercept(Chain chain) throws IOException {
		...
		TwitterServiceCredentials creds =
				realm.copyFromRealm(realm.where(TwitterServiceCredentials.class).findFirst());
		if (creds == null) creds = new TwitterServiceCredentials();
		realm.close();
		...
		if (original.url().queryParameterNames() != null && !original.url()
				.queryParameterNames()
				.isEmpty()) {
			query = new TreeMap<>();
		...
		Timber.i(rebuild);
		String signature = generateSignature(key, rebuild, original.method(), parameters, query);
		...
		for (String paramName : original.url().queryParameterNames()) {
			Timber.i(
					"Parameter: " + paramName + " " + original.url().queryParameterValues(paramName).get(0));
			newurl.addEncodedQueryParameter(percentEncode(paramName),
					percentEncode(original.url().queryParameterValues(paramName).get(0)));
		}
	...
	private String generateSignature(String key, String url, String httpMethod,
			Map<String, String> oauthParameters, Map<String, String> query) {
